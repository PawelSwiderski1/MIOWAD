...
class MLP:
    ...
    def plot_weights(self, epoch):
        for i, w in enumerate(self.weights):
            ...
            filepath = f"weights_plots/square-simple/{filename}"
    ...
    def train(
        ...
        for j in range(epochs):
            ...
            learning_rate = learning_rate_init / (1 + 0.001 * j)
...
mlp_square_1_5 = MLP([1, 15, 1], verbose=False)
training_data_scaled = [
    (x.reshape(-1, 1), y) for x, y in zip(X_train_scaled, y_train_scaled)
]
mlp_square_1_5.train(
    training_data_scaled, epochs=600, learning_rate=1, batch_size=2, plot_interval=10
)
